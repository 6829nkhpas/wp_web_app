...
import { X, Search, User, Phone, UserPlus, Loader } from 'lucide-react';
...
const AddContact = ({ isOpen, onClose, onContactAdded }) => {
  ...
  const handleAddContact = async (contact) => {
    setLoading(true);
    try {
      // In a real app, you might want to have a separate contacts API
      // For now, we'll just trigger the onContactAdded callback
  ...
  const handleStartChat = (contact) => {
    // Close modal and start chat with this contact
    onContactAdded(contact, true); // true indicates start chat immediately
    onClose();
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-[#2a3942] rounded-lg w-full max-w-md max-h-[80vh] flex flex-col">
        ...
        <div className="flex items-center justify-between p-4 border-b border-gray-600">
          <h2 className="text-white text-lg font-semibold">Add Contact</h2>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto">
          {!searchTerm.trim() ? (
            <div className="flex flex-col items-center justify-center h-full p-8 text-center">
              ...
              <p className="text-gray-400 text-lg font-medium mb-2">Find Contacts</p>
              <p className="text-gray-500 text-sm">
                Search for users by name or phone number to add them to your contacts
              </p>
            </div>
          ) : searching ? (
            ...
            <div className="flex flex-col items-center justify-center p-8 text-center">
              ...
              <p className="text-gray-400 font-medium mb-2">No users found</p>
              <p className="text-gray-500 text-sm">
                ...
                >
                  ...
                  <div className="flex space-x-2">
                    <button
                      ...
                      disabled={loading}
                      className="px-3 py-1 bg-whatsapp-green text-white rounded-lg hover:bg-whatsapp-green-dark transition-colors text-sm font-medium disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      {loading ? (
        ...
        <div className="p-4 border-t border-gray-600 bg-[#111b21]">
          <p className="text-gray-500 text-xs text-center">
            💡 Tip: You can add contacts and start chatting immediately
          </p>
        </div>
      </div>
    </div>
